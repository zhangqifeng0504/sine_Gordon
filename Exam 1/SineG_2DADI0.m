function error = SineG_2DADI0(xa,xb,ya,yb,ta,tb,m1,m2,n,alpha,beta,theta)
tic
format short e;
%%mesh
h1 = (xb-xa)/m1;
...
%% function
Rfun = @(x,y,t)  2*sin(exp(-t).*(1-cos(pi*x)).*(1-cos(pi*y)))+...
   exp(-t).*(1-cos(pi*x)).*(1-cos(pi*y))- alpha*pi^2*exp(-t).*(1-cos(pi*y)).*cos(pi*x)-...
    beta*pi^2*exp(-t).*(1-cos(pi*x)).*cos(pi*y);
varphi = @(x,y) (1-cos(pi*x)).*(1-cos(pi*y));
psi = @(x,y) -(1-cos(pi*x)).*(1-cos(pi*y));
xi =  @(x,y) 2;
psixx = @(x,y) -pi^2*cos(pi*x).*(1-cos(pi*y));
psiyy = @(x,y) -pi^2*cos(pi*y).*(1-cos(pi*x));
Rfunt = @(x,y)  -2*cos((1-cos(pi*x)).*(1-cos(pi*y))).*(1-cos(pi*x)).*(1-cos(pi*y))-...
   (1-cos(pi*x)).*(1-cos(pi*y))+ pi^2*(alpha*(1-cos(pi*y)).*cos(pi*x)+...
   beta*(1-cos(pi*x)).*cos(pi*y));

%% conditions
u(:,:,1) = (1-cos(pi*x))*(1-cos(pi*y'));
u(:,1,:) = 0; u(:,m2+1,:) = 0;
u(1,:,:) = 0; u(m1+1,:,:) = 0;
...
U = (1-cos(pi*x))*(1-cos(pi*y'))*exp(-t(2));
...
for i= 1:m1+1
    for j=1:m2+1
        ...
        %(dt^2/2)*xi(x(i),y(j))*sin(U(i,j))+(dt^2/2)*Rfun(x(i),y(j),t(1));
    end
end
...
for i = 2:m1
    ...
    u(i,2:m2,2) =  (pp(i,2:m2)+GG)/C41';%% 2 level
end
...
for k = 2:n
    ...
    for j = 2:m2
        ...
        uu(2:m1,j) = C1\(C2*u(2:m1,j+1,k)+C3*u(2:m1,j,k)+C2*u(2:m1,j-1,k)-1/12*A1*(w(2:m1,j+1).*sin(u(2:m1,j+1,k)))-...
            10/12*A1*(w(2:m1,j).*sin(u(2:m1,j,k)))-1/12*A1*(w(2:m1,j-1).*sin(u(2:m1,j-1,k)))+...
            1/12*A1*Rfun(x(2:m1),y(j+1),t(k))+10/12*A1*Rfun(x(2:m1),y(j),t(k))+...
            1/12*A1*Rfun(x(2:m1),y(j-1),t(k))+H);
    end
    ...
    for i = 2:m1
        ...
        u(i,2:m2,k+1) = dt^2*uuu(i,2:m2)+2*u(i,2:m2,k)-u(i,2:m2,k-1);%% k+1 level
    end
end

%% error
U = (1-cos(pi*x))*(1-cos(pi*y'))*exp(-t(n+1));
error = max(max(abs(U(2:m1,2:m2)-u(2:m1,2:m2,n+1))));